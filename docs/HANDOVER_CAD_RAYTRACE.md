...
平板・円筒・球の 5 体系で検証済み。差はテッセレーション（曲面の多面体近似）由来で、
警告件数もすべて一致した。**球は円筒より大きく出る**（2 方向に曲がるため逃げ道が
ない）ので、球面や複曲面が支配的な体系では偏差を下げること。

→ **実測値の表は [CAD_RAYTRACE.md](manuals/CAD_RAYTRACE.md) の
「`.paths` 経由と CSG 経由の線量比較」と「精度」を参照。** 数値はそちらを正とする
（同じ数字を複数箇所に書くと更新時に食い違うため）。
...
CSG 経由と一致することを確認した（実測値は CAD_RAYTRACE.md）。
...
| ThinnedIndices ツール | 完了。30 → 33 メソッド（その後 CAD 連携で 34、generateInput で 35） |
...
- `.paths` の第3区画が読まれないまま生成され続ける。将来の多層自動選定の材料と
  して残しているが、使われないので縮約規則にバグが入っても気づけない
