...
| 体系 | 曲面 | 検出器数 | 最大相対差 |
|---|---|---|---|
| 鉄平板（スラント補正 0/30/45/60°） | なし | 4 | **0.0052%** |
| キャスク（単一線源・点検出器） | 円筒 | 15 | 0.23% |
| キャスク（グリッド検出器 3×3 含む） | 円筒 | 24 | 0.93% |
| キャスク（2 線源） | 円筒 | 15 | 0.94% |
| 球殻 | 球 | 6 | 0.63% |

差はテッセレーション（曲面の多面体近似）由来。警告件数もすべて一致した。

**球は円筒より大きく出る。** 2 方向に曲がっているため、円筒のような「母線方向には
平坦」という逃げ道がない。球殻で偏差を変えると 0.5mm → 0.63%、0.1mm → 0.045%、
0.02mm → 0.036% と縮む。**球面や複曲面が支配的な体系では偏差を下げること。**
...
| 球面での検証 | 完了。偏差依存を確認（0.5mm で 0.63%） |
