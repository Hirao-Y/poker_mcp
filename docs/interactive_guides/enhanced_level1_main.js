...
// 改良された完了時にプレビューを表示
function showCompletionWithPreview() {
    // 既存の完了表示
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (theoryBtn) {
        theoryBtn.addEventListener('click', () => {
            // 既存の理論解説に加えて、Level 2・3の関連情報も表示
            const currentStage = appState?.currentStage || 0;
            if (currentStage >= 3) { // 後半のステージで
                setTimeout(() => {
                    const continueStudy = confirm('Level 1の基礎を習得されました。Level 2・3での学習内容をプレビューしますか？');
                    if (continueStudy) {
                        showNextLevelModal();
                    }
                }, 2000);
