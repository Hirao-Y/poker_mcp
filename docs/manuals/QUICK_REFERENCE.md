...
| **poker_deleteBody** | 立体削除 | 「test_sphereを削除」（ゾーンごと消すなら cascade=true） |
...
> 材料は `lib_material.dat` が正。大文字小文字無視で指定可（`Aluminum`→`Aluminium`）、`proposeZone` の密度省略時はカタログ密度。全材料は [MATERIAL_SYSTEM.md](./MATERIAL_SYSTEM.md) 参照。
...
標準13種＋ユーザ材料（`Source_Dry`, `SUS_A`, `Cast_Iron` 等）に対応。
...
### ✨ **日常使用パターン**
1. **34メソッド早見表**で適切なメソッド選択
2. **立体タイプ表**で形状パラメータ確認
3. **エラーコード表**で迅速トラブル解決
4. **標準ワークフロー**で効率的作業実行

### 🎯 **品質保証のポイント**
- Unit操作5メソッド・ThinnedIndices操作3メソッドで単位系完全性確保
...
**このクイックリファレンスで、34メソッド・10立体・4キー単位系を完全活用した高効率な放射線遮蔽計算を実現してください。**
