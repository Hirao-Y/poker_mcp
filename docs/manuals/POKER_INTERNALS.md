...
`CalculateDose.cpp` を読んで整理したものです。`.paths` 対応の設計検討として
書き起こしましたが、**内容は POKER が透視線をどう扱うかの解説**なので、実装が
完了した現在も参照価値があります。

`.paths` 対応は実装済みです（`--path-input`）。使い方は
[CAD_RAYTRACE.md](CAD_RAYTRACE.md)、フォーマットは
[PATHS_FORMAT.md](PATHS_FORMAT.md) を参照してください。実測値もそちらにあります。

以下は当時の検討記録を含みます。実装済みの項目には印を付けています。
