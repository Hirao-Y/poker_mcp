...
- **34メソッド完全活用**: 全機能を活用した統合設計
- **外部計算コード**: poker_cui実行・結果解析統合
- **Python自動化**: 34メソッドAPI活用による高度な自動化
...
| メソッドが見つからない | メソッド名誤り | 34メソッドリスト確認 |
...
**この統合ガイドにより、Poker MCP Server v1.8.3の34メソッド機能を最大限活用し、実用的な放射線遮蔽計算統合環境を実現できます。**
