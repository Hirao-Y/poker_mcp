...
**📚 マニュアル階層**: テクニカル層  
**🔧 対応システム**: Poker MCP Server v1.0.0  
**📅 最終更新**: 2025年9月2日
...
この文書は**テクニカル層**の完全API仕様書です。MCP (Model Context Protocol) に完全準拠した24メソッドの詳細仕様を提供します。
...
### 📡 **Detector操作系 (3メソッド) - 多次元検出器対応**

| **メソッド名** | **機能** | **検出器次元** |
|---------------|----------|---------------|
| **poker_proposeDetector** | 検出器作成 | 0D(点), 1D(線), 2D(面), 3D(体積) |
