...
**📚 マニュアル階層**: エッセンシャル層  
**🔧 対応システム**: Poker MCP Server v1.5.0 MCP Edition  
...
`DaughterReconciler.determineEquilibrium()` は親娘の半減期比で分岐する。

| 条件 | 平衡型 | 放射能係数 | 生成 |
|---|---|---|---|
| T(親) > 100 × T(子) | 永続平衡 | 分岐比 | する |
| T(親) > T(子) | 過渡平衡 | 分岐比 / (1 − T(子)/T(親)) | する |
| T(親) ≤ T(子) | 平衡なし | 決定不能 | **しない**（警告） |

**平衡が成立しない組み合わせは自動生成しない。** 冷却時間の情報なしに
放射能を決められないためで、推定値を入れると誤った線量評価につながる。
該当する場合は警告を返すので、必要ならその核種を明示的に指定する。

永続平衡の境界を 100 倍に置いているのは、この比なら過渡平衡の補正項
1/(1 − T(子)/T(親)) が 1.01 以内に収まり、実務上の有効数字で区別が
つかなくなるためである。
...
参照元:
- ${POKER_INSTALL_PATH}/LIB/ICRP-07.NDX を直接読む（v1.4.0以降）
- 娘核種を持つ核種: 1252 中 808
```
...
子孫核種:
  Ba-137m:
    ...
    崩壊: IT (γ線 662 keV)
    分岐比: 0.9439
    平衡型: 永続平衡（30.17年 / 2.552分 ≈ 6.2×10⁶ > 100）

自動生成:
- A(Ba-137m) = A(Cs-137) × 0.9439
```

**注意**: POKER のライブラリでは Cs-137 自身は光子を放出せず、662 keV は
すべて Ba-137m から出る。Cs-137 だけを指定して娘核種を除外すると、
線量がゼロになる。

### **実装例: Mo-99/Tc-99m（過渡平衡）**
```yaml
Mo-99  : 半減期 2.75日、分岐比 0.8773 → Tc-99m
Tc-99m : 半減期 6.01時間

比: 2.75日 / 6.01時間 = 11.0 → 100倍未満のため過渡平衡
係数: 0.8773 / (1 - 6.01/66.0) = 0.9654

A(Tc-99m) = A(Mo-99) × 0.9654
```

ジェネレータの溶出直後は平衡が未成立のため、この係数は適用できない。
その場合は除外登録するか、実測値を明示的に指定する。
