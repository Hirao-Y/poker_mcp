...
**POKER は二重起動できません。** 既に起動している状態で `poker_openGui` を
実行しても新しいプロセスは即座に終了し、画面は前の入力を表示したままです。

別の入力を表示するには、先に POKER ウィンドウを閉じてください。
表示中の入力を再読み込みする場合も同様です。

v1.4.0 以降は起動中を検出して `POKER_ALREADY_RUNNING` を返します。
それ以前は spawn の成否のみで判定していたため、実際には切り替わって
いないのに「起動しました」と PID 付きで報告していました。

なお `executeCalculation`（POKER_CUI）と GUI の同時実行可否は未検証です。
GUI を開いたまま計算する運用は避け、必要なら GUI を閉じてください。
