...
![FreeCAD キャスク 等角ソリッド](../assets/freecad_cask_iso.png)

*等角ソリッド表示*

![FreeCAD キャスク 断面（y>0 除去）](../assets/freecad_cask_cut.png)

*断面（y>0 を除去）：オレンジ＝線源空洞 Source_Dry、灰＝ガンマ遮蔽鉄、淡青＝中性子遮蔽ポリ、外側＝外筒鉄、上下＝鉄プラグ。この 3D モデルが POKER-MCP 経由で cm 系の RCC/CMB ボディに変換される。*
