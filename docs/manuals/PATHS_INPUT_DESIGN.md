...
**2 の再構成は実装され、動作を確認した。** CSG 経由と `.paths` 経由で線量が
0.23% 以内で一致し、80 mfp 超過の警告件数（15 件）も一致している。差は
テッセレーション由来。

採用した方式は「案A: `Calculate_Dose` の減衰積算を経路から行う」。`from_paths`
引数（既定 false）を追加し、透過長の出所だけを切り替える。複製を作らないので
CSG 経由と `.paths` 経由が同じコードを通る。

**1 の線源の区切りは未対応。** 現状は複数線源を検出したらエラーにしている。

以下は設計検討時の記録。実装済みの部分も、判断の経緯として残す。
...
1. ~~複数線源のエラーチェック~~ **完了**
2. ~~単一線源・点検出器のみで通す~~ **完了**
3. ~~CSG モデルと同じ体系で計算し線量が一致することを確認~~ **完了（0.23% 以内）**
