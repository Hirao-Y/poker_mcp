...
- **線源ごとの区切り**（`sources: [{ id, name, n_points }]`）。現状は単一線源の
  前提で、複数線源だと全点が 1 番目の線源として扱われる。区切りの情報は
  `poker_cui -p` の出力にあり、`gen_paths.py` が 1 線源分だけ読んで捨てている
- **多層ビルドアップの選定基準**。第3区画の縮約規則が確立すれば、生成側で決めた
  層構成を POKER が採用する運用もあり得る
- 経路ごとのエネルギー群指定（現状は全群共通の幾何として扱う）
- スラント補正に必要な入射角情報
- グリッド検出器の評価点への対応（現状は検出器 1 個 = 評価点 1 個の前提で
  `detector` id を振っている）
